...
/* ── Compact SVG face (FAB size) ── */
function BotFaceMini({ size = 44, eyeOffX = 0, eyeOffY = 0, blinking = false, pulse = false }: {
  size?: number; eyeOffX?: number; eyeOffY?: number; blinking?: boolean; pulse?: boolean;
}) {
  const cx = size / 2;
  const cy = size / 2;
  const r  = size / 2 - 1;
  const eyeR  = size * 0.115;
  const pupR  = size * 0.065;
  const maxOff = size * 0.055;
  const ox = Math.max(-maxOff, Math.min(maxOff, eyeOffX));
  const oy = Math.max(-maxOff, Math.min(maxOff, eyeOffY));
  const leyX = cx - size * 0.22;
  const reyX = cx + size * 0.22;
  const eyY  = cy - size * 0.05;

  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={pulse ? { animation: 'botPulse 2s ease-in-out infinite' } : {}}>
      <defs>
        <radialGradient id="bg-int" cx="40%" cy="35%" r="70%">
          <stop offset="0%" stopColor="#3b82f6" />
          <stop offset="100%" stopColor="#1e3a8a" />
        </radialGradient>
      </defs>
      <circle cx={cx} cy={cy} r={r} fill="url(#bg-int)" />
      ...
      {!blinking && <circle cx={leyX + ox} cy={eyY + oy} r={pupR} fill="#0f172a" />}
      {!blinking && <circle cx={reyX + ox} cy={eyY + oy} r={pupR} fill="#0f172a" />}
      <path
        d={`M ${cx - size * 0.18} ${cy + size * 0.2} Q ${cx} ${cy + size * 0.32} ${cx + size * 0.18} ${cy + size * 0.2}`}
        stroke="white" strokeWidth={size * 0.045} strokeLinecap="round" fill="none" opacity={0.9}
      />
      <line x1={cx} y1={1} x2={cx} y2={size * 0.14} stroke="white" strokeWidth={size * 0.045} strokeLinecap="round" opacity={0.55} />
      <circle cx={cx} cy={1.5} r={size * 0.055} fill="#60a5fa" />
    </svg>
  );
}
...
/* ── Quick action button ── */
function QuickBtn({ icon, label, value, color, onClick }: {
  icon: React.ReactNode; label: string; value?: string | number; color: string; onClick?: () => void;
}) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl text-left transition-all hover:scale-[1.02] active:scale-[0.98] border ${color}`}
    >
...
export default function BotInternal() {
  ...
  const [peeked, setPeeked] = useState(false);    // auto-peek on mount
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [statsRes, ordersRes, chatRes] = await Promise.allSettled([
        ...
        token ? fetch('/api/chat/messages', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : []) : Promise.resolve([]),
      ]);
  ...
  // Auto-peek on mount (after 1.5s) showing a summary bubble
  useEffect(() => {
  ...
  // When user opens bot manually → fetch data
  useEffect(() => {
    if (open) fetchData();
  }, [open, fetchData]);

  // Auto-peek again when unread count rises (new messages)
  ...
  useEffect(() => {
    const h = (e: MouseEvent) => {
      ...
      const factor = Math.min(dist, 280) / 280;
      setEyeOff({ x: (dx / dist) * factor * 3, y: (dy / dist) * factor * 3 });
    };
  ...
  const peekMessage = (() => {
    if (!data) return `${greeting}, ${firstName}! Loading your summary...`;
    if (unread > 0) return `Hey ${firstName}! You have ${unread} unread message${unread > 1 ? 's' : ''} 💬`;
  ...
  return (
    <>
      ...
      <style>{`@keyframes botPulse { 0%,100%{transform:scale(1)} 50%{transform:scale(1.06)} }`}</style>
      ...
      >
        <AnimatePresence>
          ...
          {open && (
            <motion.div
              ...
              className="absolute bottom-[56px] right-0 w-[300px] bg-white border border-gray-200 rounded-2xl shadow-[0_16px_48px_rgba(0,0,0,0.14)] overflow-hidden"
            >
              {/* Header */}
              <div className="flex items-center gap-2.5 px-4 py-3 border-b border-gray-100" style={{ background: 'linear-gradient(135deg,#1e3a8a,#3b82f6)' }}>
                <BotFaceMini size={32} eyeOffX={eyeOff.x} eyeOffY={eyeOff.y} blinking={blinking} />
                <div className="flex-1 min-w-0">
        ...
        {/* Peek bubble (auto on mount, does NOT open panel) */}
        <AnimatePresence>
          {peeked && !open && (
            <motion.div
              ...
              className="absolute bottom-[56px] right-[54px] bg-white border border-gray-200 rounded-2xl rounded-br-sm px-3.5 py-2.5 shadow-[0_8px_28px_rgba(0,0,0,0.12)] w-max max-w-[220px] cursor-pointer"
