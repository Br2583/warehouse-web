...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (!model.company_id) { router.replace('/onboarding'); return; }
      if (!model.profile_complete) { router.replace('/onboarding'); return; }
      router.replace('/dashboard');
    } catch (e: any) {
      const msg = e?.message || '';
      setError(msg.includes('popup') || msg.includes('closed') ? 'Sign-in window was closed. Try again.' : 'Google sign-in failed. Try again.');
      ...
      >
        ...
        <div className="p-8 md:p-12 flex flex-col justify-center overflow-y-auto">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 rounded-full border border-gray-200 bg-white text-gray-700 font-semibold text-sm hover:bg-gray-50 hover:border-gray-300 transition-all disabled:opacity-50 shadow-sm mb-4"
          >
            {googleLoading ? (
