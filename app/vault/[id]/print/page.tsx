  ...
  return (
    <>
      ...
      <style>{`
        ...
        .cb { display: inline-flex; align-items: center; justify-content: center; width: 13px; height: 13px; border: 2px solid #111827; border-radius: 2px; flex-shrink: 0; }
      ...
      <div className="vault-print-wrapper min-h-screen flex items-center justify-center p-6">
        <div id="vault-print-label" className="bg-white w-full max-w-2xl shadow-xl print:shadow-none print:max-w-none border border-gray-200" style={{ fontFamily: 'system-ui, sans-serif' }}>
          ...
          <div className="px-5 py-4 flex items-center justify-between border-b-2 border-gray-900">
            <div>
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-medium leading-none mb-0.5">Archive Contents Restoration</p>
            ...
            <div className="text-right border-2 border-gray-900 rounded-lg px-3 py-2">
              ...
              <p className="text-[10px] font-bold tracking-widest uppercase text-gray-700 mt-0.5">{levelLabel}</p>
            </div>
          </div>

          {/* ─── Hero: client + QR ─── */}
          <div className="flex flex-col md:flex-row items-start gap-4 px-5 pt-5 pb-4 border-b border-gray-300">
            <div className="flex-1 min-w-0">
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-medium mb-1">Client</p>
              <p className="text-3xl font-black text-gray-950 leading-tight break-words">{vault.client_name || '—'}</p>
              {warehouseName && (
                <p className="text-sm text-gray-600 font-medium mt-2">{warehouseName}</p>
              )}
              {nOfM && nOfM.m > 1 && (
          ...
          {/* ─── Checkboxes ─── */}
          <div className="print-grow grid grid-cols-2 gap-0 border-b border-gray-300">
            <div className="px-5 py-4 border-r border-gray-300">
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-semibold mb-3">Type of Loss</p>
            ...
            <div className="px-5 py-4">
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-semibold mb-3">Condition</p>
              <div className="space-y-2.5">
          ...
          {vault.comments && (
            <div className="px-5 py-3 border-b border-gray-300">
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-semibold mb-1">Notes</p>
          ...
          <div className="grid grid-cols-2 gap-0 border-b border-gray-300">
            <div className="px-5 py-3 border-r border-gray-300">
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-semibold mb-0.5">Packed by</p>
              <p className="text-sm font-semibold text-gray-900">{vault.packer || '—'}</p>
            </div>
            <div className="px-5 py-3">
              <p className="text-[11px] uppercase tracking-widest text-gray-600 font-semibold mb-0.5">Pack Date</p>
              <p className="text-sm font-semibold text-gray-900">{vault.pack_date ? formatDate(vault.pack_date) : '—'}</p>
            </div>
          </div>

          {/* ─── Footer ─── */}
          <div className="px-5 py-3 bg-gray-100 flex flex-wrap gap-x-4 gap-y-1 items-center">
            {warehouseName && <span className="text-xs text-gray-700">{warehouseName}</span>}
            {warehouseName && <span className="text-xs text-gray-400">·</span>}
            <span className="text-xs text-gray-700">{STATUS_LABEL[status] || status}</span>
            <span className="text-xs text-gray-400">·</span>
            <span className="text-xs text-gray-700">{vault.content_type || vault.contents_type || '—'}</span>
            {vault.job_type && (
              <>
                <span className="text-xs text-gray-400">·</span>
                <span className="text-xs text-gray-700">{vault.job_type}</span>
              </>
            )}
            <span className="ml-auto text-[9px] text-gray-400 break-all hidden print:block">{qrUrl}</span>
          </div>
