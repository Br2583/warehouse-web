  ...
  return (
    ...
    >
      ...
      <div className="bg-white/[0.06] border border-white/[0.1] rounded-2xl px-5 py-4 text-[13px] text-white/70 leading-[1.65] relative z-10 hover:bg-white/[0.1] transition-colors cursor-default">
        <strong className="text-white/90">&ldquo;Warehouse Manager cut our inventory errors by 80%.&rdquo;</strong>
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!model.company_id) {
        ...
        if (pendingAction === 'create' && pendingData) { await createCompany(model.id, pendingData, model.email || ''); return; }
        if (pendingAction === 'join'   && pendingData) { await joinCompany(pendingData); return; }
        setNeedsCompany(true); setGoogleLoading(false); return;
      }
      try {
      ...
      if (msg.includes('popup') || msg.includes('closed')) {
        setError('Sign-in window was closed. Try again.');
      } else {
            ...
            {!needsCompany && (
              <>
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 py-3 rounded-full border border-gray-200 bg-white text-gray-700 font-semibold text-sm hover:bg-gray-50 hover:border-gray-300 transition-all disabled:opacity-50 shadow-sm"
                >
                  {googleLoading ? (
